...
class TextToSpeechController extends GetxController {
  ...
  Future<void> generateText(String inputText) async {
    ...
    // Обработка ответа
    String response = chatCompletion.choices.first.message.content?.join() ?? '';
    ...
    newText = newText.substring(0, newText.length - 1); // Удаление последнего символа
    // print(newText);
  ...
  Future<void> speakText(String message) async {
    ...
    await player.play(DeviceFileSource(speechFile.path)); // will immediately start playing
